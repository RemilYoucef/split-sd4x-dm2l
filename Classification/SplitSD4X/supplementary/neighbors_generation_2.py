...
def generate_num_neighbors_2 (inst_num, n, varn):
    return np.random.multivariate_normal(inst_num,varn,n)

def generate_categ_neighbors_2 (inst_categ,n ,mat_nb_categ) :
...
def generate_all_neighbors_2(data, num_indices, categ_indices, mat_nb_categ, n_neigh) :
    
    list_neigh = []
    n = np.size(data,0)
    data_num = data[:,num_indices]
    varn = cal_varn_2 (data_num)
    
    for i in range(0,n) :
        inst_num = data_num[i]
        num_neigh_i = generate_num_neighbors_2(inst_num,n_neigh,varn)
        
        inst_categ = data[i,categ_indices]
        categ_neigh_i = generate_categ_neighbors_2(inst_categ,n_neigh ,mat_nb_categ)
