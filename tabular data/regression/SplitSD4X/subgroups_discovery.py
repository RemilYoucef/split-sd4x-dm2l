...
def SplitBasedSelectionForm (data, target, k, model,list_neigh,split_point)  :
...
def lin_models(S,data_test,list_neigh,model) :

	W = dict ()
	for s in S :
		data_neigh_s, target_neigh_s = sampling_sb(data_test,s,list_neigh,model)
		data_neigh_s = sm.add_constant(data_neigh_s, prepend=False)
		ols = sm.OLS(target_neigh_s,data_neigh_s)
		W[s] = ols.fit()
		del ols
...
def lin_models_for_sim(S,data_test,list_neigh,model) :
